...
complete -c tally -n "__fish_tally_needs_command" -f -a "scan" -d 'Scan for task updates from git commits, source markers, and TODO.md entries'
...
complete -c tally -n "__fish_tally_using_subcommand help; and not __fish_seen_subcommand_from add done list semver remove yank scan help" -f -a "scan" -d 'Scan for task updates from git commits, source markers, and TODO.md entries'
